...
entity tb_spi_byte_random is
    generic(
        NUM_TESTS_G : integer := 100
    );
end entity;

architecture scoreboard of tb_spi_byte_random is
    ...
    signal tx_scoreboard_s : spi_transaction_array_t(0 to NUM_TESTS_G - 1);
    signal rx_scoreboard_s : spi_transaction_array_t(0 to NUM_TESTS_G - 1);
    ...
    begin
        ...
        -- result := std_logic_vector(to_unsigned(integer(rand_v * real(2**width-1)), width));
...
begin
    ...
    stim_proc : process
        ...
        variable seed1_v        : positive:=197;
        variable seed2_v        : positive:=49;
    begin
        ...
        -- MOSI SFR Loop Test
        ----------------------------------------------------------------
        for i in 0 to NUM_TESTS_G - 1 loop

            random_slv(seed1_v, seed2_v, exp_byte_v'length, exp_byte_v);

            tx_byte_i           <= exp_byte_v;
            tx_scoreboard_s(i).expected  <= exp_byte_v;
            --wait until rising_edge(clk_i);
            start_i     <= '1';
            wait until rising_edge(clk_i);
            start_i     <= '0';
    ...
    begin
        ...
        for i in 0 to NUM_TESTS_G - 1 loop
            ...
            tx_shift_s                  <= exp_byte_v;
            rx_scoreboard_s(i).expected <= exp_byte_v;
